...
if($var ==="Todas"){
  ...
  $consulta = "where s.id_evaluador > 0 and s.eliminada = 0";
  $consultados = "where s.eliminada = 0 and s.id_evaluador > 0";
  $consultatres = "where s.eliminada = 0 and s.id_evaluador > 0 and s.evaluado = 1";
  $consultacuatro = "where s.eliminada = 0 and s.id_evaluador > 0 and s.evaluado in (0, '')";
}else{
  $consulta = "where oi.direccion = '$var' and s.eliminada = 0"; 
  $consultados = "where s.eliminada = 0 and s.id_evaluador > 0 and ww.direccion = '$var'";
  $consultatres = "where s.eliminada = 0 and s.id_evaluador > 0 and s.evaluado = 1 and ww.direccion = '$var'";
  $consultacuatro = "where s.eliminada = 0 and s.id_evaluador > 0 and s.evaluado in (0, '') and ww.direccion = '$var'";
}
...
left join workbeat as ww on ww.id_slack = s.id_slack 
$consultados ";
