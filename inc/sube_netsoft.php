...
// }else{
	if(move_uploaded_file($guardado, 'archivos/'.$nombre)){
		$nombreArchivo = 'archivos/google_netsoft.xlsx';
...
$licenses	= $objPHPExcel->getActiveSheet()->getCell('AK'.$i)->getCalculatedValue();
$new_licenses	= $objPHPExcel->getActiveSheet()->getCell('AL'.$i)->getCalculatedValue();
$advanced_protection_program_enrollment	= $objPHPExcel->getActiveSheet()->getCell('AM'.$i)->getCalculatedValue();

		
		 $expiration_timestamp = $objPHPExcel->getActiveSheet()->getCell('J'.$i)->getCalculatedValue();
		
		 $sql = mysqli_query ($conexion,"SELECT email_address  from google_netsoft where email_address = '$email_address'");
		
		 $result= mysqli_num_rows($sql);
		 if ($result > 0) { 
			
			
			 $sqlup = "UPDATE google_netsoft
			SET  			
			first_name = '$first_name',
			last_name = '$last_name',
			google_netsoft.password = '$password',
			password_hash_function = '$password_hash_function',
			org_unit_path = '$org_unit_path',
			new_primary_email = '$new_primary_email',
			google_netsoft.status = '$status',
			...
			if($resultup){
			 ...
			 $sqlinser = "INSERT INTO google_netsoft
			(
			first_name,
			last_name,
			email_address,
			google_netsoft.password,
			password_hash_function,
			org_unit_path,
			new_primary_email,
			google_netsoft.status,
			last_sign_in,
			recovery_email,
			...
			if($resultinser){
				$sqlupids = "UPDATE google_netsoft  
				INNER JOIN slack on upper (SUBSTRING(slack.email, 1,(instr(slack.email,'@')-1 ))) = upper (SUBSTRING(google_netsoft.email_address, 1,(instr(google_netsoft.email_address,'@')-1 )))
				SET google_netsoft.id_slack = slack.id_slack 
                    where google_netsoft.email_address = '$email_address'";
