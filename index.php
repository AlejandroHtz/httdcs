...
if (!empty($_SESSION['active'])) {
  if($_SESSION['permisos'] < 4){
    echo "<script> window.location='inc/list_evaluar.php'</script>";
}else { echo "<script> window.location='inc/mievaluacion.php'</script>";}
  ...
  if (!empty($_POST)) {
    if (empty($_POST['usuario'])||empty($_POST['clave'])) {
      ...
      if ($result > 0) {
        ...
        $_SESSION['id_slack']=$data['id_slack'];
        $_SESSION['permisos']=$data['permisos'];
        $_SESSION['evaluador']=$data['evaluador'];
        $_SESSION['gerente']=$data['gerente'];
